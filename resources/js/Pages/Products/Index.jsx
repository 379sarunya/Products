...
export default function Index({ products }) { // นิยามฟังก์ชัน Index ที่รับ prop ชื่อ products
    return (
        <div className="bg-[#F1E8FD]  min-h-screen"> {/* กำหนดพื้นหลังและความสูงเต็มหน้าจอ */}
                ...
                <div className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8"> {/* กำหนดการจัดเรียงสินค้าด้วยกริด */}
                    {products.map((product) => ( // วนลูปผ่าน products เพื่อแสดงสินค้าทุกตัว
                        <a
                            ...
                            className="group shadow-lg hover:shadow-xl transition-shadow duration-300 rounded-lg bg-[#D9C4EC]" // ตั้งค่ารูปลักษณ์ของลิงค์ รวมถึงเงาเมื่อ hover
                        >
                            <img
                                src={product.image} // ใช้ภาพจากข้อมูลของผลิตภัณฑ์
                                alt={product.name} // กำหนดคำอธิบายภาพ
                                className="aspect-square w-full rounded-t-lg bg-[#BEA9DF] object-cover group-hover:opacity-90 xl:aspect-[7/8]" // กำหนดขนาดและสไตล์ของภาพ
                            />
                            <div className="p-4"> {/* บล็อกสำหรับข้อมูลภายในที่ใช้ padding รอบๆ */}
                                ...
                                <p className="mt-2 text-lg font-semibold text-black-800"> {/* ราคาสินค้า */}
                                    ฿{product.price}
                                </p>
                            </div>
                        </a>
